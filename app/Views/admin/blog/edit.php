...
<main class="container py-4">
  ...
  <form method="POST" action="<?= base_url('admin/blog/update/' . $blog['blog_id']) ?>" enctype="multipart/form-data" autocomplete="off">

    <!-- Blog Image Upload -->
    <div class="form-group mb-3">
      <label for="blog_image"><strong>Blog Image</strong></label>
      <input type="file" class="form-control-file" name="blog_image" id="blog_image" accept="image/*">
      <?php if (!empty($blog['blog_image'])): ?>
        <div class="mt-2">
          <img src="<?= base_url('uploads/blog/' . $blog['blog_image']) ?>" alt="Current Blog Image" style="height: 80px;">
        </div>
      <?php endif; ?>
    </div>
    ...
    <div id="editorWrapper">
      <textarea id="ckeditor" class="form-control"><?= old('description', $blog['description']) ?></textarea>
    </div>
